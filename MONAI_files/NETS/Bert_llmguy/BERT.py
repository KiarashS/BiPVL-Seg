...
from .modules.layers_ours import Linear_d
...
class BertLayer(nn.Module):
    ...
    def feed_forward_chunk(self, ao1, ao2):
        ...
        # layer_output = layer_output * matrix (768, 768)
        return layer_output
...
class BertLayer_debias(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        ...
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        self.P = torch.nn.Parameter(torch.eye(config.hidden_size) + 0.01 * torch.randn(config.hidden_size, config.hidden_size))
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = BertAttention(config, position_embedding_type="absolute")
    ...
    def feed_forward_chunk(self, ao1, ao2):
        intermediate_output = self.intermediate(ao1)
        layer_output = self.output(intermediate_output, ao2)
        layer_output = torch.matmul(layer_output, self.P)
        return layer_output
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers):
            # Create a new BertLayer with the given configuration
            layer = BertLayer(config)
            if i == 8: layer = BertLayer_debias(config)
            layers.append(layer)
