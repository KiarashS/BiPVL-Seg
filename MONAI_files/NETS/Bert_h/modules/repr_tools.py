...
def get_words_idxs_in_templates(
    tok: BertTokenizer, context_templates: str, words: str, subtoken: str
) -> int:
    """
...
def get_reprs_at_idxs(
    ...
    def _process(cur_repr, batch_idxs, key):
        ...
        # print(f"test-- {cur_repr[0][8].shape}")
        # print(f"current batch_dix : {batch_idxs}")
        for _, (i, idx) in enumerate(batch_idxs):
            to_return[key].append(cur_repr[i][idx])
    ...
    if len(to_return["out"]) == 0: print("Warning: no output representations found")
...
def get_module_input_output_at_words(
    ...
    elif fact_token_strategy == "mask":
        mask_tok_idxs = [[(i, idx) for idx, tok_id in enumerate(tok(ctxt, padding=True).input_ids) if tok_id == tok.mask_token_id] for i, ctxt in enumerate(contexts)]
        mask_tok_idxs = [idxs for sublist in mask_tok_idxs for idxs in sublist]  # Flatten list
        # print('here is the contexts: ', contexts)
        # print('here is the mask_tok_idxs: ', mask_tok_idxs)
        l_in, l_out = get_reprs_at_idxs(
            contexts=contexts,
            idxs=mask_tok_idxs,
            **word_repr_args,
        )
        # print("Selected u projection token at mask position")
    else:
