# %%
model_type = "unet" # natunetr, dinatunetr, unet, unet++, segresnet, nnunet, attunet, unetr, swinunetr, nnformer, medformer, mednext, universal, zept, cat, mulmodseg
dataset = "brain" # whs_ct, whs_mri, amos, brain
seed = 42
natunetr = False
roi_size = (96,96,96)
...
# %%
# train_ds = CacheDataset(train, transform=train_transforms,  cache_rate=1.0, num_workers=num_workers)
# val_ds = CacheDataset(data=val, transform=val_transforms,  cache_rate=1.0, num_workers=num_workers)

# train_ds = Dataset(train, transform=train_transforms)
...
ASD = SurfaceDiceMetric(include_background=True, class_thresholds=[2]*(num_channels), use_subvoxels=True)       #Average surface distance
...
with torch.no_grad():
    for val_data in tqdm(test_loader):
        ...
        val_outputs = sliding_window_inference(
            inputs=val_inputs, roi_size=roi_size, sw_batch_size=1,
            predictor=model, device=device, overlap=0.25
        )
        val_outputs = [post_pred(i) for i in decollate_batch(val_outputs)]
        # val_labels = [post_label(i) for i in decollate_batch(val_labels)]
    ...
    print(f"{model_type} on {dataset} dataset")    
    metric = dice_metric.aggregate().item()
